do_desc <- function(
  ...
  desc <- mrg_dat_pop[, .(
    ...
    avgmeantemp = round(mean(meantemp), 1),
    year = year
 ), 
 by = .(state, date)]
 ...
 rates_reshaped <- dcast(rates_by_sex, state + date ~ sex, value.var = "rate")
 setnames(rates_reshaped, c("M", "F"), c("rate_m", "rate_f"))
 
 # Merge with the previous result
 descriptive <- merge(desc, rates_reshaped, by = c("state", "date"))
 
 descriptive[, date := as.Date(paste(date, "01", sep="-"), format="%Y-%m-%d")]
 
 descriptive <- descriptive[descriptive$state != "Other", ]
